...
$(document).ready(function() {
	...
	if($('#add-new').length > 0){
		$('#add-new').on('hide.bs.modal', function() {
			$('#add-new form')[0].reset();
			$('#add-new form input[type=hidden]').val('');
		}).on('show.bs.modal', function(e) {
			var act = $("#add-new form").data('action');
		  if(e.relatedTarget) act += '/add';
		  else act += '/edit';
		  $("#add-new form").attr('action',act);
		});
